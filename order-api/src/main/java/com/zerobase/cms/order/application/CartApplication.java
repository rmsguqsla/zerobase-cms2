        ...
        for (int i = 0; i < cart.getProducts().size(); i++) {
            ...
            for (int j = 0; j < cartProduct.getItems().size(); j++) {
                ...
                if (isPriceChanged && isCountNotEnough) {
                    tmpMessages.add(cartProduct.getName() + " 가격변동, 수량이 부족하여 구매 가능한 최대치로 변경되었습니다.");
                } else if (isPriceChanged) {
                    tmpMessages.add(cartProduct.getName() + " 가격이 변동되었습니다.");
                } else if (isCountNotEnough) {
                    tmpMessages.add(cartProduct.getName() + " 수량이 변동되었습니다.");
                }
            }
